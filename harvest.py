...
headers = {
    'Origin': 'http://kibanaapplogsprod-us.trtaprofdevops.services',
    ...
    'Referer': 'http://kibanaapplogsprod-us.trtaprofdevops.services/_plugin/kibana/app/kibana',
...
data1 = '{"index":"' +  "onvio-*" + '","ignore_unavailable":true}'
...
while(start_date_ms < epoch_end_ms):
    ...
    # OLD {"range":{"#_traac-timestamp":{"gte":'+str(start_date_ms)+',"lte":'+str(end_date_ms)+'}}}
        j1 = json.loads(data1)
        j2 = json.loads(data2)

        d = json.dumps(j1) + "\n" + json.dumps(j2) + "\n"
        
        response = requests.post(msearch, headers=headers, params=params, cookies=cookies, data=d)
